---
title: "Microprocessor Labs"
toc: false
format: html
---
<p class="justified-indent"> 
   In Fall 2025, I was a part of the [E155: Microprocessor Systems](https://hmc-e155.github.io/) upperdivision technical course at Harvey 
   Mudd College. In this class, we designed and built complex digital electronic systems using tools such as FPGAs, microcontrollers, and 
   custom hardware components. Through a series of hands-on labs and design challenges, I developed skills in RTL design, embedded programming, 
   and hardware-software integration. Please check out my work below—each lab was completed to the highest standard, earning an `Excellence` evaluation.
  </p>

By clicking the 
  <button style="border-radius: 6px; padding: 8px 14px; background-color: #f5f5f5; border: 1px solid #aaa; cursor: pointer;">
    Learn More About
  </button>
buttons below, see the full technical report and verification for each lab. 

## Lab 1: FPGA & MCU Setup & Testing ##

<div style="display: flex; flex-direction: row; flex-wrap: wrap; gap: 20px;">
<div style="flex: 2; width: 300px;">
<img src="images/COVER_1.png" alt="Lab 2 Breadboard Setup" style="width:100%; border-radius: 15px;">
</div>


<div style="flex: 2; min-width: 300px;">
  <p class="justify-text"> 
    __Overview:__ <br>
      In this lab, components were soldered to an FPGA and designs were implemented on the FPGA to demonstrate the functionality of the 
      on-board high-speed oscillator to light up three on-board LEDs in specific sequences and demonstrate hexademical counting from 
      `0` to `F` on a 7-segment display. 
  </p>

  <a href="E155_Lab_1.qmd" target="_blank" style="text-decoration: none;">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #f5f5f5; border: 1px solid #aaa; cursor: pointer;">
      Learn More About Lab 1
    </button>
  </a>

  <a href="https://www.youtube.com/watch?v=OBn2m9Y9EUg&feature=youtu.be" target="_blank">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #ff0000; color: white; border: none; font-weight: bold; cursor: pointer;">
      ▶ Lab 1: FPGA Video
  </button>
  </a>
</div>
</div>

## Lab 2: Time Multiplexing ##

<div style="display: flex; flex-direction: row; flex-wrap: wrap; gap: 20px;">
<div style="flex: 2; width: 300px;">
<img src="images/COVER_2.png" alt="Lab 2 Breadboard Setup" style="width:100%; border-radius: 15px;">
</div>

<div style="flex: 2; min-width: 300px;">
  <p class="justify-text"> 
    __Overview:__ <br>
      In this lab, time-divsion multiplexing (DTM) was implemented to drive a single dual 7-segment display with only nine FPGA I/O pins. The
      hexadecimal values displayed on each half of the dual 7-segment display were added and the sum was displayed as a 5-bit binary number on 
      five external LEDs. Notably, PNP transistors were used to drive sufficient current to power the common anodes of the dual 7-segment 
      display while respecting the FGPA's current capacity. 
  </p>

  <a href="E155_Lab_2.qmd" target="_blank" style="text-decoration: none;">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #f5f5f5; border: 1px solid #aaa; cursor: pointer;">
      Learn More About Lab 2
    </button>
  </a>

  <a href="https://www.youtube.com/watch?v=jsT2PJhZEE0&feature=youtu.be" target="_blank">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #ff0000; color: white; border: none; font-weight: bold; cursor: pointer;">
      ▶ Lab 2: Multiplexing Video
    </button>
  </a>
</div>
</div>

## Lab 3: Keypad Scanner ##

<div style="display: flex; flex-direction: row; flex-wrap: wrap; gap: 20px;">
<div style="flex: 2; width: 300px;">
<img src="images/COVER_3.png" alt="Lab 3 Keypad Setup" style="width:100%; border-radius: 15px;">
</div>

<div style="flex: 2; min-width: 300px;">
  <p class="justify-text"> 
    __Overview:__ <br>
     In this lab, a design was implemened on an an FPGA to scan a 4x4 keypad matrix for key presses and display the selected presses as hexadecimal numbers `0` to `F`
     on a dual 7-segment display, with the most recent entry appearing on the right. When a key is pressed, it is recorded exactly once no matter how long 
     or short the user holds the key down. Moreover, if additional keys are pressed while still holding down the first key, the additional keys are ignored. 
  </p>

  <a href="E155_Lab_3.qmd" target="_blank" style="text-decoration: none;">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #f5f5f5; border: 1px solid #aaa; cursor: pointer;">
      Learn More About Lab 3
    </button>
  </a>

  <a href="https://youtube.com/shorts/XlhlLJF7L-8?si=OlkXNgqgWlVvEJsm" target="_blank">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #ff0000; color: white; border: none; font-weight: bold; cursor: pointer;">
      ▶ Lab 3: Keypad Video
    </button>
  </a>
</div>
</div>

## Lab 4: Digital Audio ##

<div style="display: flex; flex-direction: row; flex-wrap: wrap; gap: 20px;">
<div style="flex: 2; width: 300px;">
<img src="images/COVER_4.png" alt="Lab 4 Breadboard Setup" style="width:100%; border-radius: 15px;">
</div>

<div style="flex: 2; min-width: 300px;">
  <p class="justify-text"> 
    __Overview:__ <br>
      In this lab, the internal peripheral timers of the [STM32L432KC MCU](https://hmc-e155.github.io/assets/doc/ds11451-stm32l432kc.pdf) were used to 
      drive a small speaker to play music. To implement this function, the PWM timers on the MCU generated square waves to toggle a GPIO pin at a specific 
      frequency and manage note duration. The MCU was programmed to play *Für Elise* by Beethoven and *Blue (Da Be Dee)* by Eiffel 65. 
  </p>

  <a href="E155_Lab_4.qmd" target="_blank" style="text-decoration: none;">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #f5f5f5; border: 1px solid #aaa; cursor: pointer;">
      Learn More About Lab 4
    </button>
  </a>

  <a href="https://www.youtube.com/watch?v=cxhU5bDPtMg&feature=youtu.be" target="_blank">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #ff0000; color: white; border: none; font-weight: bold; cursor: pointer;">
      ▶ Lab 4: Für Elise & Blue Video
    </button>
  </a>
</div>
</div>

## Lab 5: Interrupts ##

<div style="display: flex; flex-direction: row; flex-wrap: wrap; gap: 20px;">
<div style="flex: 2; width: 300px;">
<img src="images/COVER_5.png" alt="Lab 5 Breadboard Setup" style="width:100%; border-radius: 15px;">
</div>

<div style="flex: 2; min-width: 300px;">
  <p class="justify-text"> 
    __Overview:__ <br>
      In this lab, the [STM32L432KC MCU](https://hmc-e155.github.io/assets/doc/ds11451-stm32l432kc.pdf) was configured with interrupts to measure the rotational speed and direction 
      of a [25GA-370](https://www.tronsunmotor.com/data/upload/file/201910/e78fcf93ed604a64c69852b5db49a03f.pdf) brushed DC motor containing a quadrature encoder. Pulses from the 
      encoder were detected on the rising and falling edges of both encoder signals to achieve a high degree of accuracy in real-time. 
  </p>

  <a href="E155_Lab_5.qmd" target="_blank" style="text-decoration: none;">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #f5f5f5; border: 1px solid #aaa; cursor: pointer;">
      Learn More About Lab 5
    </button>
  </a>

  <a href="https://www.youtube.com/shorts/_3dX2f85GPs?si=QGl4zoE-1pl1GI4M" target="_blank">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #ff0000; color: white; border: none; font-weight: bold; cursor: pointer;">
      ▶ Lab 5: Encoder Video
    </button>
  </a>
</div>
</div>

## Lab 6: IoT & SPI ##

<div style="display: flex; flex-direction: row; flex-wrap: wrap; gap: 20px;">
<div style="flex: 2; width: 300px;">
<img src="images/COVER_6.png" alt="Lab 6 Breadboard Setup" style="width:100%; border-radius: 15px;">
</div>

<div style="flex: 2; min-width: 300px;">
  <p class="justify-text"> 
    __Overview:__ <br>
     In this lab, an Internet of Things (IoT) device was designed and implemented using the [STM32L432KC MCU](https://hmc-e155.github.io/assets/doc/ds11451-stm32l432kc.pdf)
    interfaced with a [DS1722](https://www.analog.com/media/en/technical-documentation/data-sheets/ds1722.pdf) digital temperature sensor over serial peripheral interface (SPI) and an 
    [ESP8266](https://cdn-shop.adafruit.com/product-files/2471/0A-ESP8266__Datasheet__EN_v4.3.pdf) WiFi module via USART. The purpose of the IoT device was to generate a webpage that 
    displayed real-time ambient temperature readings (ºC) of the lab room, while allowing users to toggle an LED and select temperature resolution.
  </p>

  <a href="E155_Lab_6.qmd" target="_blank" style="text-decoration: none;">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #f5f5f5; border: 1px solid #aaa; cursor: pointer;">
      Learn More About Lab 6
    </button>
  </a>

  <a href="https://youtu.be/i5u0AH16xaY" target="_blank">
    <button style="border-radius: 6px; padding: 8px 14px; background-color: #ff0000; color: white; border: none; font-weight: bold; cursor: pointer;">
      ▶ Lab 6: Temperature & LED Video
    </button>
  </a>
</div>
</div>